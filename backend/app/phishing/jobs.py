...
from app.phishing.service import run_phishing_experiment_for_api


FINAL_STATUSES = {"completed", "failed", "cancelled", "interrupted"}


class PhishingJobCancelled(RuntimeError):
    pass


class PhishingExperimentJobManager:
    def __init__(self, *, max_workers: int = 1, job_dir: Path | None = None) -> None:
        ensure_dirs()
        self.job_dir = job_dir or (RESULTS_DIR / "phishing_jobs")
        self.job_dir.mkdir(parents=True, exist_ok=True)
        self._lock = Lock()
        self._executor = ThreadPoolExecutor(max_workers=max_workers, thread_name_prefix="phishing-experiment")
    ...
    def submit(
        ...
        *,
        ...
        job_id = f"phishing_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}_{uuid4().hex[:8]}"
        ...
        job = {
            ...
            "progress": {
                ...
                "fold": None,
                "modelId": None,
                "message": "Experimento de phishing en cola.",
            },
            "resultRunId": None,
            "preflight": None,
            "error": None,
    ...
    def resume(self, job_id: str) -> dict[str, Any] | None:
        with self._lock:
            ...
            if previous["status"] not in {"failed", "cancelled", "interrupted"}:
                raise ValueError("Solo se puede reanudar una corrida fallida, cancelada o interrumpida.")
            ...
            if active_same_run:
                raise ValueError("Ya existe una reanudación activa para esta corrida.")
    ...
    def list(self, limit: int = 25) -> dict[str, Any]:
        with self._lock:
            return {"items": deepcopy(sorted(self._jobs.values(), key=lambda item: item["createdAt"], reverse=True)[:limit])}
        ...
        def progress(event: dict[str, Any]) -> None:
            with self._lock:
                ...
                if job["cancelRequested"]:
                    raise PhishingJobCancelled("La corrida fue cancelada por el usuario.")
                ...
                if event.get("preflight") is not None:
                    job["preflight"] = event["preflight"]
                if event.get("executionRunId"):
                    job["executionRunId"] = event["executionRunId"]
                self._persist_unlocked(job)

        try:
            result = run_phishing_experiment_for_api(
                ...
                demo_max_rows=config.get("demoMaxRows"),
            ...
            self._update(
                ...
                progress={**self.get(job_id)["progress"], "stage": "completed", "event": "completed", "percent": 100.0, "message": "Modelos base OOF completados."},
            )
        except PhishingJobCancelled as exc:
            ...
            self._update(job_id, status="failed", finishedAt=_now(), error=str(exc), progress={**self.get(job_id)["progress"], "stage": "failed", "message": "La ejecución terminó con error."})
    ...
    def _restore_jobs(self) -> None:
        for path in self.job_dir.glob("*.json"):
            ...
            if job.get("status") in {"queued", "running"}:
                ...
                job["finishedAt"] = _now()
                job["error"] = "El backend se reinició antes de finalizar la corrida."
                job["progress"]["stage"] = "interrupted"
                job["progress"]["message"] = job["error"]
                self._persist_unlocked(job)
            job.setdefault("executionRunId", job.get("jobId"))
            job.setdefault("resumedFromJobId", None)
            job.setdefault("preflight", None)
            job.setdefault("progress", {}).setdefault("resumedUnits", 0)
...
phishing_job_manager = PhishingExperimentJobManager(max_workers=1)
