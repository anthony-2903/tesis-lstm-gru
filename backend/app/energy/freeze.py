...
class EnergyFreezeGateError(ValueError):
    def __init__(self, audit: dict[str, Any]) -> None:
        self.audit = audit
        super().__init__("La corrida energetica no supera la puerta de congelacion.")


def audit_energy_freeze_readiness(
    *,
    base_manifest_path: Path | None = None,
    revalidation_manifest_path: Path | None = None,
    verify_artifacts: bool = False,
) -> dict[str, Any]:
    base_path = base_manifest_path or _latest_base()
    revalidation_path = revalidation_manifest_path or (base_path.parent / "energy_revalidation_v1" / "revalidation_manifest.json")
    ...
    observed_grid = {(int(row.get("seed", -1)), int(row.get("fold", -1)), str(row.get("modelId"))) for row in base.get("baseFoldMetrics", [])}
    data = get_energy_dataset_status()
    source = base.get("dataset", {}).get("sourceLineage", {})

    _check(checks, "base_status", base.get("status") == "thesis_candidate", "thesis_candidate", base.get("status"))
    _check(checks, "real_dataset_ready", bool(data.get("readyForThesisPilot")), "OPSD real apto", data.get("readyForThesisPilot"))
    _check(checks, "dataset_lineage", source.get("verified") is True and source.get("datasetId") == data.get("datasetId") and source.get("sha256") == data.get("silver", {}).get("sha256"), "datasetId/hash actuales", source)
    _check(checks, "five_seeds", len(seeds) >= 5, ">=5", sorted(seeds))
    _check(checks, "five_folds", len(folds) >= 5, ">=5", sorted(folds))
    _check(checks, "five_models", models == set(THESIS_MODEL_IDS), list(THESIS_MODEL_IDS), sorted(models))
    ...
    ensemble_ids = {str(row.get("ensembleId")) for row in base.get("ensembles", {}).get("aggregate", [])}
    _check(checks, "ensemble_candidates", ensemble_ids == {"mean", "weighted_mean", "stacking_gradient_boosting"}, ["mean", "weighted_mean", "stacking_gradient_boosting"], sorted(ensemble_ids))
    _check(checks, "base_test_lock", base.get("walkForward", {}).get("testSetUsed") is False and base.get("aggregate", {}).get("testSetEvaluatedOnce") is False, "test no usado", base.get("walkForward", {}))
    _check(checks, "anomaly_scope_disclosed", base.get("anomalies", {}).get("labelType") == "estimated" and base.get("anomalies", {}).get("classificationMetricsAvailable") is False, "anomalias estimadas sin afirmar F1", base.get("anomalies", {}))
    ...
    _check(checks, "revalidation_status", revalidation.get("status") == "thesis_optimized_candidate", "thesis_optimized_candidate", revalidation.get("status"))
    ...
    ablations = {str(row.get("ablationId")) for row in revalidation.get("ablation", {}).get("configurations", [])}
    expected_ablations = {"full", *(f"without_{model}" for model in THESIS_MODEL_IDS)}
    _check(checks, "complete_ablation", ablations == expected_ablations, sorted(expected_ablations), sorted(ablations))
    ranking = set(independent.get("ranking", []))
    _check(checks, "independent_candidate_table", ranking == {*THESIS_MODEL_IDS, "optimized_stacking"}, sorted({*THESIS_MODEL_IDS, "optimized_stacking"}), sorted(ranking))
    seed_metric_pairs = {(int(row.get("seed", -1)), str(row.get("predictorId"))) for row in independent.get("seedMetrics", [])}
    expected_seed_metric_pairs = {(seed, predictor) for seed in seeds for predictor in {*THESIS_MODEL_IDS, "optimized_stacking"}}
    _check(checks, "independent_seed_stability", independent.get("seedCount", 0) >= 5 and seed_metric_pairs == expected_seed_metric_pairs, len(expected_seed_metric_pairs), len(seed_metric_pairs))
    inference = independent.get("pairedInference", {})
    _check(checks, "paired_day_bootstrap", inference.get("iterations", 0) >= 300 and inference.get("calendarDays", 0) > 1, "bootstrap diario >=300", inference)
    _check(checks, "xai_available", bool(revalidation.get("xai", {}).get("items")), "importancias del metamodelo", revalidation.get("xai", {}))
...
def create_energy_freeze_package(*, base_manifest_path: Path | None = None, revalidation_manifest_path: Path | None = None) -> dict[str, Any]:
    audit = audit_energy_freeze_readiness(base_manifest_path=base_manifest_path, revalidation_manifest_path=revalidation_manifest_path, verify_artifacts=True)
    if not audit["ready"]:
        raise EnergyFreezeGateError(audit)
    freeze_id = f"energy_freeze_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}"
    destination = RESULTS_DIR / "energy_freezes" / freeze_id
    ...
    configuration = {
        ...
        "anomalyLabels": "estimated; classification claims are prohibited until labeled ground truth exists",
    }
...
def get_latest_energy_freeze() -> dict[str, Any]:
    paths = sorted((RESULTS_DIR / "energy_freezes").glob("*/freeze_manifest.json"), key=lambda path: path.stat().st_mtime, reverse=True)
    if not paths:
        return {"available": False, "message": "No existe congelamiento energetico."}
...
def _latest_base() -> Path:
    paths = find_energy_manifests()
    if not paths:
        return EXPERIMENTS_DIR / "missing" / "energy_oof_v1" / "oof_manifest.json"
    return paths[0]
