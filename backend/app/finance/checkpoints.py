...
class FinanceCheckpointCompatibilityError(ValueError):
    pass


class FinanceOOFCheckpointStore:
    """Atomic checkpoints for one financial model, temporal fold and seed."""
    ...
    def __init__(
        ...
        *,
        ...
        if self.manifest_path.exists():
            ...
            if manifest.get("fingerprint") != fingerprint or manifest.get("contract") != contract:
                raise FinanceCheckpointCompatibilityError(
                    "El checkpoint financiero pertenece a otro dataset o configuración."
                )
    ...
    def load_unit(
        ...
        *,
        ...
        try:
            ...
            if unit.get("scalerSha256") != scaler_sha256:
            ...
            if artifacts.get("predictions", {}).get("sha256") != prediction_hash or artifacts.get("predictions", {}).get("bytes") != prediction_bytes:
                raise ValueError("artefacto de predicciones inválido")
            if artifacts.get("model", {}).get("sha256") != model_hash or artifacts.get("model", {}).get("bytes") != model_bytes:
                raise ValueError("artefacto de modelo inválido")
            frame = pd.read_csv(predictions_path)
            if list(frame.columns) != ["transaction_id", "is_fraud", "probability"]:
                raise ValueError("contrato de predicciones inválido")
            if frame["transaction_id"].astype(np.int64).tolist() != transaction_ids:
                raise ValueError("cobertura de transacciones incompatible")
            if not np.array_equal(frame["is_fraud"].to_numpy(dtype=np.int32), labels):
    ...
    def save_unit(
        ...
        *,
        ...
        prediction_frame = pd.DataFrame(
            {
                ...
                "is_fraud": labels.astype(np.int32),
        ...
        prediction_hash, prediction_bytes = sha256_file(predictions_path)
        model_path = self._model_path(seed, fold, model_id)
        actual_model_hash, actual_model_bytes = sha256_file(model_path)
        if actual_model_hash != model_sha256 or actual_model_bytes != model_bytes:
            raise ValueError("El modelo financiero guardado no coincide con su hash.")
        unit = {
            "schemaVersion": "1.0.0",
            "unitKey": key,
            ...
            "rows": len(transaction_ids),
            "metrics": metrics,
            "artifacts": {
                "model": {"path": str(model_path.resolve()), "sha256": actual_model_hash, "bytes": actual_model_bytes},
                "predictions": {
    ...
    def mark_completed(self, *, final_manifest_path: Path, final_manifest_sha256: str) -> None:
        ...
        self.manifest["finalManifest"] = {"path": str(final_manifest_path.resolve()), "sha256": final_manifest_sha256}
        self._persist_manifest()
